...
const baseUrl = "http://localhost:8080/service/getServicesType/";
class UserServices extends React.Component {
  ...
  componentWillMount() {
      ...
      axios.get(baseUrl+this.props.match.params.location).then((res) => {
        this.setState({
  ...
  navigateToCompanies = (service) =>{
    const {userId} = this.state;
    const {
      history: { push },
    } = this.props;
    push(`/viewAllServices/${this.props.match.params.location}/${service}`);
  ...
  <div style={{backgroundColor:'#000080',display:'flex',justifyContent:'center',padding:'12px',borderBottom:'1px solid white',fontSize:'24px',color:'white'}}>Appointment Booking </div>
      ...
      <div className="flex flex-col relative w-full" style={{marginTop:'55px'}}>
       ...
       { projectDatas.map((projectData) => (
         ...
         borderRadius: '4px'}} onClick={e=>this.navigateToCompanies(projectData)}>
          {projectData}
          </div>
        ))
        }
