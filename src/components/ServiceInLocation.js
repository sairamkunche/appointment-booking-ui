...
const baseUrl = "http://localhost:8080/service/getService/";
class ServiceInLocation extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    ...
    axios.get(baseUrl+this.props.match.params.location+"/"+this.props.match.params.service).then((res) => {
      this.setState({
...
updateDashboard=()=>{
  axios.get(baseUrl+this.props.match.params.location+"/"+this.props.match.params.location).then((res) => {
    this.setState({
      projectDatas:res.data
    })
})
  ...
  navigateToBookings = (id) =>{
    const {userId} = this.state;
    const {
      history: { push },
    } = this.props;
    push(`/viewServiceDetails/${id}`);
  ...
  <div style={{backgroundColor:'#000080',display:'flex',justifyContent:'center',padding:'12px',borderBottom:'1px solid white',fontSize:'24px',color:'white'}}>Appointment Booking </div>
      ...
      <div className="flex flex-col relative w-full" style={{marginTop:'55px'}}>
        ...
        { projectDatas.map((projectData) => (
         ...
         borderRadius: '4px'}} onClick={e=>this.navigateToBookings(projectData.id)}>
...
export default withRouter(ServiceInLocation);
